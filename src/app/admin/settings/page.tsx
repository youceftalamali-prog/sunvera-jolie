...
const SECTIONS: { key: string; label: string; fields: [string, string, string?][] }[] = [
  { key: "store", label: "Store Information", fields: [["name", "Store name"], ["tagline", "Tagline"], ["logoUrl", "Logo URL"], ["faviconUrl", "Favicon URL"], ["email", "Email"], ["phone", "Phone"], ["whatsapp", "WhatsApp"], ["address", "Address"]] },
...
export default function AdminSettingsPage() {
  ...
  const load = useCallback(async () => {
    ...
    const d = (await res.json()) as { settings: Settings; theme: Theme };
  ...
  async function saveSection(key: string, patch: Record<string, unknown>) {
    ...
    setMsg(res.ok ? `${key} settings saved ✓` : "Could not save settings");
  }
